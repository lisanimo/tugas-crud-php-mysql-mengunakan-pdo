...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE kontac SET id = ?, nama = ?, email = ?, telpon = ?, pekerjaan = ? WHERE id = ?');
        $stmt->execute([$id, $nama, $email, $telpon, $pekerjaan, $_GET['id']]);
        $msg = 'Updated Successfully!';
    }
    // Get the contact from the contacts table
    $stmt = $pdo->prepare('SELECT * FROM kontac WHERE id = ?');
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <input type="text" name="telpon" value="<?=$contact['telpon']?>" id="notelp">
